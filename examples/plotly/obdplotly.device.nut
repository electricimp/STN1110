...
// sends log updates to server in batches
class BatchLogger {
    _log = null // log table
    ...
    function subscribe(key) {
        ...
        return (function(value) {
            if(!"err" in value) { // if there is an error just reuse the last value
...
// connect to the STN1110
car <- VehicleInterface(hardware.uart57)
// create a batch logger
logger <- BatchLogger()

// subscribe the logger to various vehicle parameters and update once every 5 seconds
car.subscribe(car.ENGINE_RPM logger.subscribe("rpm"), 5)
car.subscribe(car.VEHICLE_SPEED, logger.subscribe("speed"), 5)
car.subscribe(car.THROTTLE_POSITION, logger.subscribe("throttle"), 5)
car.subscribe(car.ENGINE_RUNTIME, logger.subscribe("runtime"), 5)
car.subscribe(car.COOLANT_TEMPERATURE, logger.subscribe("coolant_temp"), 5)
car.subscribe(car.INTAKE_AIR_TEMPERATURE, logger.subscribe("intake_temp"), 5)
car.subscribe(car.FUEL_PRESSURE, logger.subscribe("fuel_pressure"), 5)